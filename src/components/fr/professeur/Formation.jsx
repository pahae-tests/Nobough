...
export default function Formation({ formationID }) {
    ...
    const formation = {
        ...
        type: "Avie",
        tags: ["JavaScript", "WordPress", "Web", "Frontend"],
        genre: 'tous',
    ...
    const etudiants = [
        {
            ...
            birth: "22/07/1998",
            bio: "test test test test test test test test test test test test test test test test test test test test",
    ...
    const handleAddDocument = (e) => {
        e.preventDefault()
        setShowAddDoc(false)
        const { titre, description, file } = newDocument
        if (!titre || !description || !file) return
        ...
        reader.onloadend = () => {
            ...
            setNewDocument({ titre: '', description: '', file: null })
        }
        reader.readAsDataURL(file)
    }

    const handleAddRemark = (studentId, remark) => {
        setStudentRemarks(prevRemarks => ({
            ...prevRemarks,
            [studentId]: remark
        }))
        const updatedStudents = etudiants.map(student =>
            student.id === studentId ? { ...student, remarque: remark } : student
        )
    }
    ...
    return (
        <>
                ...
                {activeTab === 'bibliotheque' && (
                    <div className="bg-white/60 backdrop-blur-xs p-4 sm:p-6 rounded-xl shadow-xl mb-6">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-2">
                            ...
                            <button
                                className="bg-green-600 text-white rounded-lg p-3 hover:bg-green-700 transition-colors cursor-pointer"
                            ...
                            {etudiants.map((etudiant) => (
                                <div key={etudiant.id} className="bg-white rounded-xl shadow-sm overflow-hidden">
                                    ...
                                    <div className="border-t px-4 py-3">
                                        ...
                                        {
                                            etudiant.remarque && <div className="mb-3">
                                                <h4 className="font-medium text-sm mb-1 text-green-500">Remarque</h4>
                                                <p className="text-sm text-gray-700">{etudiant.remarque}</p>
                                            </div>
                                        }
                                        ...
                                        <div className="mt-4">
                                            <form
                                                onSubmit={(e) => {
                                                    ...
                                                    handleAddRemark(etudiant.id, remark)
                                                    e.target.elements.remark.value = ''
                                                }}
                                            ...
                                            >
                                                ...
                                                <button
                                                    type="submit"
                                                    className="bg-pink-600 text-white rounded-lg p-2 hover:bg-pink-700 transition-colors cursor-pointer"
                                                >
                                                    Ajouter
                                                </button>
                                            </form>
            ...
            {showAddDoc && (
                <div className='fixed inset-0 top-0 left-0 cursor-pointer bg-black/50 backdrop-blur-md z-50 flex justify-center items-center' onClick={() => setShowAddDoc(false)}>
                    <form onSubmit={handleAddDocument} className="mt-4 p-4 bg-white/60 backdrop-blur-xs shadow-x rounded-lg shadow-sm" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="mb-4">
                            ...
                            <input
                                ...
                                className="mt-1 p-2 border rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-pink-500"
                                required
                            />
                        </div>
                        <div className="mb-4">
